...
log = logging.getLogger("red.sky.althell")


class alt_hell(commands.Cog):
    """A quarantine suspicious users."""

    def __init__(self, bot):
        self.bot = bot
        self.config = Config.get_conf(self, 2700081002, force_registration=True)
    ...
    async def _strip_roles(self, ctx, user):
        ...
        await self.bot.get_channel(1010547668306624543).send(embed=e)
        await user.edit(
...
#
    ...
    async def banish(self, ctx, user: discord.Member):
        """Strip a user of their roles, apply the alt_hell role, and blacklist them.
		If the blacklist toggle is off, the user will not be blacklisted from the bot."""
        data = await self.config.guild(ctx.guild).all()
        blacklist = data["auto_blacklist"]
        alt_hell_role_id = data["alt_hell_role"]
        alt_hell_role_obj = discord.utils.get(ctx.guild.roles, id=alt_hell_role_id)
        ...
        if not alt_hell_role_obj:
            return await ctx.send("No alt_hell role set.")
        try:
            await self._strip_roles(ctx, user)
            await user.add_roles(
                alt_hell_role_obj, reason=f"Adding alt_hell role, {ctx.message.author} is stripping user"
            )
        except discord.Forbidden:
            await ctx.send(
        ...
        if blacklist:
            ...
            blacklist_msg = ""
        msg = f"{user} has been sent to the alt_hell{blacklist_msg} and has had all previous roles stripped."
        await ctx.send(msg)

    @commands.group(autohelp=True)
    @commands.guild_only()
    @checks.admin_or_permissions(manage_guild=True)
    async def alt_hell(self, ctx):
        """Main alt_hell commands."""
        pass

    @alt_hell.command()
    async def announce(self, ctx, channel: discord.TextChannel):
        """Sets the announcement channel for users moved to the alt_hell."""
    ...
    async def autoban(self, ctx):
        """Toggle auto-banning users instead of sending them to the alt_hell."""
        auto_ban = await self.config.guild(ctx.guild).auto_ban()
        await self.config.guild(ctx.guild).auto_ban.set(not auto_ban)
        await ctx.send(f"Auto-ban instead of sending to the alt_hell: {not auto_ban}.")
    ...
    async def blacklist(self, ctx):
        """Toggle auto-blacklisting for the bot for users moved to the alt_hell."""
        ...
        await ctx.send(f"Auto-blacklisting alt_hell users from the bot: {not auto_blacklist}.")
    ...
    async def bypass(self, ctx, user_id: int):
        ...
        await ctx.send(
            f"**{bypassed_user}** ({user_id}) has been added to the bypass list. The next time they attempt to join this server, they will bypass any alt_hell restrictions."
        )
    ...
    async def channel(self, ctx, channel: discord.TextChannel):
        """Sets the text channel to use for the alt_hell."""
        await self.config.guild(ctx.guild).alt_hell_channel.set(channel.id)
        alt_hell_channel_id = await self.config.guild(ctx.guild).alt_hell_channel()
        await ctx.send(f"alt_hell channel set to: {self.bot.get_channel(alt_hell_channel_id).name}.")
    ...
    async def joindays(self, ctx, days: int):
        ...
        await ctx.send(
            f"Users must have accounts older than {days} day(s) to be awarded the member role instead of the alt_hell role on join."
        )
    ...
    async def role(self, ctx, role_name: discord.Role):
        """Sets the role to use for the alt_hell."""
        await self.config.guild(ctx.guild).alt_hell_role.set(role_name.id)
        alt_hell_role_id = await self.config.guild(ctx.guild).alt_hell_role()
        alt_hell_role_obj = discord.utils.get(ctx.guild.roles, id=alt_hell_role_id)
        await ctx.send(f"alt_hell role set to: {alt_hell_role_obj.name}.")

    @alt_hell.command()
    async def toggle(self, ctx):
        """Toggle the alt_hell on or off."""
        alt_hell_enabled = await self.config.guild(ctx.guild).toggle()
        await self.config.guild(ctx.guild).toggle.set(not alt_hell_enabled)
        await ctx.send(f"alt_hell enabled: {not alt_hell_enabled}.")
    ...
    async def userrole(self, ctx, *, role_name: discord.Role = None):
        """Sets the role to give to new users that are not sent to the alt_hell."""
        if not role_name:
    ...
    async def usertoggle(self, ctx):
        ...
        await ctx.send(f"New user alt_hell role enabled: {not user_role_toggle}.")

    @alt_hell.command()
    async def free(self, ctx, user: discord.Member):
        """Verify a user: remove the alt_hell role and add initial user role."""
        data = await self.config.guild(ctx.guild).all()
        ...
        for role in user.roles:
            if not alt_hell_role_obj:
                return await ctx.send("No alt_hell role set.")
            if role == alt_hell_role_obj:
                role_check = True
                try:
                    await user.remove_roles(
                        alt_hell_role_obj,
                        reason=f"Removing alt_hell role, verified by {ctx.message.author}.",
                    )
                    """
                    if not user_role_obj:
                        return await ctx.send(
                            "alt_hell role removed, but no member role is set so I can't award one."
                        )
        ...
        if not role_check:
            return await ctx.send("User is not in the alt_hell.")

        if blacklist:
            blacklist_msg = " and the bot blacklist"
        else:
            blacklist_msg = ""
        msg = f"{user} has been removed from the alt_hell{blacklist_msg} and now has their roles back."
        await ctx.send(msg)
    ...
    async def autosetup(self, ctx):
        """Automatically set up the alt_hell channel and role to apply to suspicious users.
        ...
        try:
            ...
            alt_hell_role = await ctx.guild.create_role(name="alt_hell")

            alt_hell_category = await ctx.guild.create_category("alt_hell", overwrites=overwrites)
            await alt_hell_category.set_permissions(
                alt_hell_role, read_messages=True, send_messages=False, read_message_history=True
            )

            alt_hell_channel = await ctx.guild.create_text_channel(
                "Silenced", category=alt_hell_category
            )
            await alt_hell_channel.set_permissions(
                alt_hell_role, read_messages=True, send_messages=False, read_message_history=True
            )
            await alt_hell_channel.set_permissions(
                ctx.guild.me, read_messages=True, send_messages=True, read_message_history=True
            )
            await alt_hell_channel.send("Please wait while an admin verifies your account.")

            await self.config.guild(ctx.guild).alt_hell_channel.set(alt_hell_channel.id)
            await self.config.guild(ctx.guild).alt_hell_role.set(alt_hell_role.id)
            await self.config.guild(ctx.guild).announce_channel.set(ctx.channel.id)
            toggle = await self.config.guild(ctx.guild).toggle()
            if not toggle:
                await ctx.invoke(self.usertoggle)
            await ctx.send(
                f"Done.\nalt_hell channel created: {alt_hell_channel.mention}\nalt_hell role created: {alt_hell_role.name}\n\nPlease set these items manually:\n- The announce channel for reporting new users that are moved to the alt_hell ({ctx.prefix}alt_hell announce)\n- The role you wish to award regular members when they join the server ({ctx.prefix}alt_hell userrole)\n- The toggle for enabling the regular user role awarding ({ctx.prefix}alt_hell usertoggle)"
            )
    ...
    async def settings(self, ctx):
        ...
        try:
            drole = discord.utils.get(ctx.guild.roles, id=data["alt_hell_role"]).name
        ...
        try:
            ...
            achannel = None
        try:
            dchannel = self.bot.get_channel(data["alt_hell_channel"]).name
        except AttributeError:
            dchannel = None
        alt_hell_enabled = data["toggle"]
        ...
        msg = (
            "```ini\n----alt_hell Settings----\n"
            f"alt_hell Enabled:  [{alt_hell_enabled}]\n"
            f"alt_hell Role:     [{drole}]\n"
            f"alt_hell Channel:  [{dchannel}]\n"
...
#
    ...
    async def on_member_join(self, member):
        ...
        if member.id in bypassed_ids:
            ...
            try:
                await member.add_roles(
                    user_role_obj,
                    reason="User has bypassed alt_hell checks. Assigning member role.",
                )
            except discord.Forbidden:
                pass
            except AttributeError:
                pass
            bypass_msg = f"**{member}** ({member.id}) was in the bypass list for **{member.guild.name}** ({member.guild.id}). They were allowed to join without alt_hell checks and I have assigned them the Member role specified in the settings, if any."
            if announce_channel:
                await channel_object.send(bypass_msg)
            else:
                log.debug(f"alt_hell.py: {bypass_msg}")
            return
        ...
        if (since_join.days < join_days) or (profile_toggle and default_avatar):
            ...
            alt_hell_role_id = await self.config.guild(member.guild).alt_hell_role()
            alt_hell_role_obj = discord.utils.get(member.guild.roles, id=alt_hell_role_id)
            perm_msg = f"alt_hell.py: Unable to auto-ban user, permissions needed and no announce channel set. Guild: {member.guild.id}"
            ...
            if auto_ban:
                ...
                try:
                    await member.guild.ban(
                        member, reason="alt_hell auto-ban", delete_message_days=0
                    )
                except discord.Forbidden:
                    if announce_channel:
                ...
                if not mod_log:
                    ...
                    try:
                        ...
                        log.error(
                            f"alt_hell.py error while autobanning user and attempting to create modlog entry in guild: {member.guild.id}",
                            exc_info=True,
                        )
            ...
            try:
                if since_join.days < join_days:
                    reason = "Adding alt_hell role, new account."
                else:
                    reason = "Adding alt_hell role, default profile pic."
                await member.add_roles(alt_hell_role_obj, reason=reason)
            except discord.Forbidden:
                if announce_channel:
                    return await channel_object.send(
                    ...
                    log.info(
                        f"alt_hell.py: I need permissions to manage channels and manage roles in {member.guild.name} ({member.guild.id})."
                    )
                    return
