...
class Raffle(BaseCog):
    """Run simple Giveaways for your server."""
    ...
    async def start(self, ctx, timer, *, title: str):
        ...
        Only one giveaway can be active per server.
        """
        ...
        try:
            ...
            return await ctx.send("Response timed out. A giveaway failed to start.")
        ...
        embed.set_footer(
            text=(
                f"Started by: {ctx.author.name} | Winners: {winners} | Ends at {fmt_end} UTC | giveaway ID: {msg.id}"
            )
        )
    ...
    async def end(self, ctx, message_id: int = None):
        """Ends a giveaway early. A winner will still be chosen."""
        if message_id is None:
            try:
                message_id = await self._menu(ctx)
            except ValueError:
                return await ctx.send("There are no active giveaways to end.")
            except asyncio.TimeoutError:
        ...
        try:
            ...
            await ctx.send("The giveaway has been ended.")

    @giveaway.command()
    async def cancel(self, ctx, message_id: int = None):
        """Cancels an on-going giveaway. No winner is chosen."""
        if message_id is None:
            try:
                message_id = await self._menu(ctx, end="cancel")
            except ValueError:
                return await ctx.send("There are no active giveaways to cancel.")
            except asyncio.TimeoutError:
                return await ctx.send("Response timed out.")
        ...
        try:
            ...
            await ctx.send("The giveaway has been canceled.")
        finally:
    ...
    async def _menu(self, ctx, end="end"):
        title = f"Which of the following **Active** giveaways would you like to {end}?"
        async with self.config.guild(ctx.guild).Raffles() as r:
            if not r:
                raise ValueError
    ...
    async def reroll(self, ctx, channel: discord.TextChannel, messageid: int):
        """Reroll the winner for a giveaway. Requires the channel and message id."""
        try:
    ...
    async def setgiveaway(self, ctx):
        """Set giveaway group command"""
        pass

    @setgiveaway.command()
    async def channel(self, ctx, channel: discord.TextChannel = None):
        """Set the output channel for giveaway."""
        if channel:
            await self.config.guild(ctx.guild).Channel.set(channel.id)
            return await ctx.send(f"giveaway output channel set to {channel.mention}.")
        await self.config.guild(ctx.guild).Channel.clear()
        await ctx.send("giveaways will now be started where they were created.")
    ...
    async def raffle_setup(self, ctx):
        ...
        q4 = "Do you want to limit this giveaway to specific roles?"
    ...
    async def raffle_worker(self):
        """Restarts giveaway timers
        This worker will attempt to restart giveaway timers incase of a cog reload or
    ...
    async def raffle_timer(self, guild, raffle: dict, remaining: int):
        """Helper function for starting the giveaway countdown.

        This function will silently pass when the unique giveaway id is not found or
        if a giveaway is empty. It will call `raffle_teardown` if the ID is still
        ...
        guild : Guild
            The guild object
        raffle : dict
            All of the giveaway information gained from the config to include:
    ...
    async def pick_winner(self, guild, channel, msg):
        ...
        if reaction is None:
            return await channel.send(
                "It appears there were no valid entries, so a winner for the giveaway could not be picked."
            )
        ...
        try:
            ...
            return await channel.send("An error occurred, so a winner for the giveaway could not be picked.")
