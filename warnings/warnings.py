...
class Warnings(commands.Cog):
    ...
    async def addnote(
        ...
        *,
        ...
        """Add a mod note to the specified member.
        `<reason>` is reason for the warning. This can be a registered reason,
        or a custom reason if ``[p]warningset allowcustomreasons`` is set.
        """
        guild = ctx.guild
        ...
        if toggle_channel:
            ...
            if warn_channel:
                ...
                await ctx.send(
                    _("A note has been added to {user}.").format(user=member.mention), embed=em
                )
        else:
            await ctx.tick()
        reason_msg = _(
            ...
            if not user_warnings.keys():  # no warnings for the user
                ...
                await ctx.send_interactive(
                    ...
                    box_lang=_("Warnings for {user}").format(
                        user=str(member) if isinstance(member, discord.Member) else member.id
                    ),
                )
